...
export function AddressForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (autoLabel !== originalLabel) {
            let suffix = 2;
            while (addresses.some(a => a.label === autoLabel)) {
                autoLabel = `${editing.name.trim()} (${suffix++})`;
            }
        }
        const submitData = { ...editing, label: autoLabel, originalLabel };
