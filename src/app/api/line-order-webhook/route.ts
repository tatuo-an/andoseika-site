...
規格（2L/L/優/大/小など）が書かれていない、または「小と大が混ざる可能性がある」のように曖昧な場合でも、規格について聞き返さないでください。お客様が書いた品名をそのまま（例:「白ネギ」「5kg規格外小」）data.itemsのnameに入れ、数量だけで確定してよいものとして扱ってください。質問してよいのは数量や配達日など、品名・規格以外の情報が本当に読み取れない場合だけです。
