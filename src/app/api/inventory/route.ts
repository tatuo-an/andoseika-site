...
export async function POST(req: NextRequest) {
    ...
    const { items } = await req.json() as {
        items: { id: string; name: string; stock: number; price: number | null; shipType: string; hidden: boolean; nextShipment: string; badges: string[]; family: string; imageUrl?: string; familyImages?: string[]; cost?: number | null; profitRate?: number | null; coolAvailable?: boolean; description?: string; clickpostMax?: number; options?: string; salePercent?: number; saleStart?: string; saleEnd?: string; shipMode?: string; shipValue?: string; compactMax?: number; category?: string }[];
    };
